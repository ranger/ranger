...
=item draw_borders_rounded [bool]

Use the unicode characters "╭ ╰ ╮ ╯" to draw borders with rounded corners.
...
=item idle_auto_freeze [integer]

The number of seconds without user input before ranger automatically enters
frozen mode.  See I<freeze_files> setting.
...
=item multipane_orientation [string]

Set layout orientation in multipane view mode to either of:

 vertical       traditional side-by-side layout
 horizontal     stack browser widgets on top of each other
