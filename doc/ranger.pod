...
starting ranger. Alternatively, plugins may be built as installable python
packages, which may be registered with ranger by adding an entry point in the
C<ranger.plugins> group. The value should point to the module containing the
plugin. To do this, you could write your F<setup.py> file similar to the
following:

 from setuptools import setup

 setup(
     name="my-ranger-plugin",
     packages=["my_ranger_plugin"],
     # the following makes a plugin available to ranger
     entry_points={"ranger.plugins": ["plugin_name = myproject.pluginmodule"]},
     # custom PyPI classifier for ranger plugins
     classifiers=[
         "Environment :: Plugins",
     ],
 )

A plugin changes rangers behavior by overwriting or extending a function that
ranger uses.  This allows you to change pretty much every part of ranger, but
there is no guarantee that things will continue to work in future versions as
