...
=item quick_jump_letters

The characters in the quick_jump_letters strings are used as ids
in the case that the quick jump flag is activated in the scout command.
Active ids can not be used in the console, while the quick jump mode
is active, but the quick mode can be paused via 
fm.ui.quick_jump.toggle_paused(), which is bind to the < key by default.
Another possible setup is to use upper case character in combination
with the -i flag, so that the lower case characters can
be used for filtering, while the upper case characters can be used
to quickly move to files.

=item quick_jump_reverse_seq

Reverse the character order of the quick jump id.
...
=item copyqmap I<key> I<newkey> [I<newkey2> ...]
...
=item qmap I<key> I<command>

Binds keys for the quick jump mode of the scout command. Overwrites
the console key bindings, but for unbound keys, the console mapping
is still in use. Works like the C<map> command.
...
Removes key mappings of the quick jump mode. Works like the C<unmap> command.
