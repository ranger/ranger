...
Use a user-specified command C<user_previewer> for displaying image previews.
...
=item user_previewer [string]

User-defined command for generating image previews.

The following labels are automatically interpolated:
...
Width and height (respectively) of the display area in columns and rows.

=item C<{fit_width}>, C<{fit_height}>
Width and height (respectively) of the display area in pixels.

=item C<{lcb}>, C<{rcb}>
Left and right curly brackets respectively.
