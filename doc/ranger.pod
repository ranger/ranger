...
The characters in the quick_jump_letters string are used as ids in the
case that the quick-jump flag is activated in the scout command.
While the quick jump mode is active, id-characters can not be used in
the console. But the quick mode can be paused via
fm.ui.quick_jump.smart_tab(), which is bind to the <tab> key by
default.
