...
iTerm's image protocol requires the image file to be split into chunks, the size
of which is set by the option C<iterm2_multipart_file_chunk_size>.  It is set to
a sensible default, but can be tweaked for compatibility in certain situations.
Versions of C<tmux> before 2.0 may need this set to 256.  Versions of iTerm
before 3.5.0 will need this set to 0, which reverts to iTerm's older image
protocol.
...
=item iterm2_multipart_file_chunk_size [integer]

The maximum allowable length of escape sequences used when rendering iTerm2
image previews.  Set to 0 for no limit, which will be necessary for versions
of iTerm2 prior to 3.5.0.  Set to 256 when using versions of tmux prior to
2.0.
