               ...
               && [ "${SQLITE_ROW_LIMIT}" -ge 0 ]; then
                ...
                sqlite3 "file:${FILE_PATH}?mode=ro" -noheader \
                    ...
                    while read -r sqlite_table; do
                        sqlite_table_rowcount="$(
                          sqlite3 "file:${FILE_PATH}?mode=ro" -noheader \
                            "SELECT count(*) FROM ${sqlite_table}"
                        )"
                        if [ "${SQLITE_ROW_LIMIT}" -gt 0 ] \
                           && [ "${SQLITE_ROW_LIMIT}" \
                              -lt "${sqlite_table_rowcount}" ]; then
                            echo
                            echo "${sqlite_table} [${SQLITE_ROW_LIMIT} of ${sqlite_table_rowcount}]:"
                            ...
                            sqlite_show_query \
                              'SELECT *
                               FROM (SELECT * FROM '"${sqlite_table}"' LIMIT 1)
                               UNION ALL '"${sqlite_ellipsis_query}"'
                               UNION ALL SELECT * FROM (
                                 SELECT * FROM '"${sqlite_table}"'
                                   LIMIT ('"${SQLITE_ROW_LIMIT}"' - 1)
                                   OFFSET ('"${sqlite_table_rowcount}"'
                                          - ('"${SQLITE_ROW_LIMIT}"' - 1)
                                          )
                               );'
                        else
                            echo
                            echo "${sqlite_table} [${sqlite_table_rowcount}]:"
